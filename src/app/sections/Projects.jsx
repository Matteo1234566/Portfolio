...
const projects = [
  {
    ...
    description: 'Real-time sports analytics platform. Uses advanced computer vision to track player movements and ball trajectory from video streams automatically.',
  ...
  },
  {
    ...
    title: 'Traid',
    category: 'FinTech AI',
    description: 'AI-driven trading platform utilizing neural time-series models to predict market trends and visualize data in a real-time dashboard.',
  ...
  },
  {
    ...
    description: 'Advanced speech-to-text pipeline designed for academic transcription, handling multi-speaker identification and specialized terminology.',
  ...
  },
  {
    ...
    title: 'Wi-Fi Sensing',
    category: 'R&D',
    description: 'Environmental perception system using standard Wi-Fi signals to detect human presence and movement without cameras.',
  ...
  return (
    <div className="max-w-6xl mx-auto px-4">
      <div className="flex flex-col md:flex-row justify-between items-end mb-12">
        ...
        >
           <h2 className="font-display text-5xl font-bold mb-2 text-ink dark:text-white leading-tight">
             ...
             <span className="relative inline-block">
               <span className="relative z-10 px-2 text-ink">WORK</span>
               <motion.span 
        ...
        >
          We snap our fingers,<br/>your product comes alive.
        </motion.p>
      </div>
      ...
      >
        {projects.map((project) => (
          <motion.div key={project.id} variants={item} className="group h-full">
            <Card className={`h-full flex flex-col ${project.highlight ? 'border-bubblegum border-4' : ''}`}>
              <div className="flex justify-between items-start mb-4">
                <span className="font-mono text-xs font-bold text-forest bg-green-100 px-2 py-1 rounded uppercase">
                  {project.category}
                </span>
                <ExternalLink className="text-ink/30 dark:text-white/30 group-hover:text-ink dark:group-hover:text-white transition-colors" size={20} />
              </div>
              ...
              <div className="pt-6 border-t-2 border-smoke dark:border-white/10 flex flex-wrap gap-2">
                {project.techStack.map(tech => (
                  <span key={tech} className="text-sm font-bold text-ink/50 dark:text-white/50">
                    #{tech}
                  </span>
                ))}
              </div>
